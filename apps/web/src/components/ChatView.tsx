...
export default function ChatView(props: ChatViewProps) {
  ...
  const completionDividerBeforeEntryId = useMemo(() => {
    ...
    return deriveCompletionDividerBeforeEntryId(displayedTimelineEntries, activeLatestTurn);
  }, [activeLatestTurn, completionSummary, displayedTimelineEntries, latestTurnSettled]);
  ...
  const handleThreadIdentityChange = useCallback(
    async (identity: ThreadIdentity) => {
      ...
      if (!api) {
        setThreadError(activeThread.id, "Thread API unavailable.");
        return;
      }
      ...
      try {
        ...
        setThreadError(
          activeThread.id,
          error instanceof Error ? error.message : "Failed to update thread identity.",
        );
      }
    },
