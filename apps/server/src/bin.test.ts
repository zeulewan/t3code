...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const relocatedWorkspaceRoot = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "t3-cli-projects-relocated-workspace-"),
        );
    ...
    Effect.gen(function* () {
      ...
      yield* withLiveProjectCliServer(baseDir, () =>
        Effect.gen(function* () {
          ...
          const videoPath = NodePath.join(workspaceRoot, "clip.mp4");
          NodeFS.writeFileSync(videoPath, Buffer.from("fake mp4 test payload"));
  ...
  it.effect("rejects legacy positional comms sender arguments", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "t3-cli-comms-legacy-sender-test-"),
      );
  ...
  it.effect("supports developer-only comms sender override with --from", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "t3-cli-comms-from-flag-test-"),
      );
  ...
  it.effect("requires developer override when passing explicit comms sender", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "t3-cli-comms-from-guard-test-"),
      );
  ...
  it.effect("sends comms with an autodetected sender handle", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "t3-cli-comms-autodetect-test-"),
      );
